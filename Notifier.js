...
var pluginList = ['cpu', 'memory', 'disk', 'process', 'offline'];
...
slack.on('message', function (message) {
    ...
    if (type === 'message' && (text != null) && (channel != null)) {
	    ...
	    //host validation check
            ...
            var pluginType = commands[2];
            logger.info('type='+pluginType);
            exists = false;
            for(id in pluginList){
                if(pluginType === pluginList[id]){
                    exists = true;
                }
            }
            ...
            if(!exists)
            {
                 logger.error('플러그인타입이 존재하지 않습니다');
                 channel.send('플러그인타입이 존재하지 않습니다');
                 return;
            }
...
function sendSMS(host, type, typeInstance, grade, value, timestamp) {
    oracledb.getConnection({
        ...
        function (err, connection) {
            ...
            var issuedTime = new Date().getToday();

            switch (type) {
                case 'offline':
                    message = '[' + host + '] ADFMonitoring Agent가 Offline 상태입니다. '+issuedTime;
                break;
                case 'cpu':
                    message = '[' + host + '] CPU사용률(' + value + ')이 높습니다.(ADF) '+issuedTime;
                break;
                case 'vmemory':
                    message = '[' + host + '] 가상메모리사용률(' + value + ')이 높습니다.(ADF) '+issuedTime;
                break;
                case 'df':
                    message = '[' + host + '] ' + typeInstance + ' 파일시스템의 사용된 공간백분율(' + value + ')이 높습니다.(ADF) '+issuedTime;
                break;
                case 'process':
                    message = '[' + host + '] ' + typeInstance + ' 프로세스가 kill되었거나 존재하지 않습니다.(ADF) '+issuedTime;
                break;
                case 'online':
                    message = '[' + host + '] ADFMonitoring Agent가 Online 상태로 전환 되었습니다. '+issuedTime;
                break;
            }
